    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input): SV_Target
            {
                ...
                float4 var_MaskTex = SAMPLE_TEXTURE2D(_MaskMap, sampler_BaseMap, input.uv);
                float3 Albedo = var_BaseTex.rgb * _BaseColor.rgb;
                float Opacity = var_BaseTex.a;
                float Metallic = var_MaskTex.r;
                float Glossiness = var_MaskTex.g;
                float AO = var_MaskTex.b;
                ...
                float3 IndirSpeCubeFactor = IndirSpeFactor(roughness, Glossiness, BRDF, F0, NdotV);
